...
export default {
    ...
    methods: {
        getWeather() {
            ...
            axios.get(`${process.env.API_URL}/api/v1/weather/days?city=${this.cityName}`)
        ...
        deleteWeather(index, id) {
            axios.delete(`${process.env.API_URL}/api/v2/weather/${id}`)
                .then(() => {
                    this.weathers.splice(index, 1)
                })
        },
        changeWeather(index, id, weather) {
            axios.put(`${process.env.API_URL}/api/v2/weather/${id}`, weather)
                .then((res) => {
        ...
        saveWeather() {
            axios.get(`${process.env.API_URL}/api/v1/weather/db/curr?city=${this.cityName}`)
                .then(() => {
                    this.good1 = true
                    this.getCities()
                    this.getWeathers()
        ...
        saveFiveDaysWeather() {
            axios.get(`${process.env.API_URL}/api/v1/weather/db/days?city=${this.cityName}`)
                .then(() => {
                    this.good5 = true
                    this.getCities()
                    this.getWeathers()
        ...
        deleteUser(index, id) {
            axios.delete(`${process.env.API_URL}/api/v2/user/${id}`)
                .then(() => {
                    this.users.splice(index, 1)
                })
        },
        changeUser(index, id, newName) {
            axios.put(`${process.env.API_URL}/api/v2/user/${id}`, { "name": newName })
                .then(res => {
        ...
        deleteCity(index, id) {
            axios.delete(`${process.env.API_URL}/api/v2/city/${id}`)
                .then(() => {
                    this.cities.splice(index, 1)
                })
        },
        changeCity(index, id, newName) {
            axios.put(`${process.env.API_URL}/api/v2/city/${id}`, { "cityName": newName })
                .then(res => {
        ...
        getUserCities() {
            axios.get(`${process.env.API_URL}/api/v2/user/${this.selectedUser}`)
                .then(res => {
