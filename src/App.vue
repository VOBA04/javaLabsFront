...
export default {
    ...
    methods: {
        getWeather() {
            axios.get(`${API_URL}/api/v1/weather/curr?city=${this.cityName}`)
                .then((res) => {
            ...
            axios.get(`${API_URL}/api/v1/weather/days?city=${this.cityName}`)
                .then((res) => { this.fiveDaysWeather = res.data })
        },
        getWeathers() {
            axios.get("${API_URL}/api/v2/weather/all/sorted")
                .then((res) => {
                    this.weathers = res.data
                })
        },
        addWeather() {
            axios.post("${API_URL}/api/v2/weather/", {
                "temp": this.temp,
        ...
        deleteWeather(index, id) {
            axios.delete(`${API_URL}/api/v2/weather/${id}`)
                .then(() => {
                    this.weathers.splice(index, 1)
                })
        },
        changeWeather(index, id, weather) {
            axios.put(`${API_URL}/api/v2/weather/${id}`, weather)
                .then((res) => {
        ...
        saveWeather() {
            axios.get(`${API_URL}/api/v1/weather/db/curr?city=${this.cityName}`)
                .then(() => {
                    this.good1 = true
                    this.getCities()
                    this.getWeathers()
        ...
        saveFiveDaysWeather() {
            axios.get(`${API_URL}/api/v1/weather/db/days?city=${this.cityName}`)
                .then(() => {
                    this.good5 = true
                    this.getCities()
                    this.getWeathers()
        ...
        getUsers() {
            axios.get("${API_URL}/api/v2/user/all")
                .then((res) => {
                    this.users = res.data
                })
        },
        addUser() {
            axios.post("${API_URL}/api/v2/user/", { "name": this.userName })
                .then((res) => {
        ...
        deleteUser(index, id) {
            axios.delete(`${API_URL}/api/v2/user/${id}`)
                .then(() => {
                    this.users.splice(index, 1)
                })
        },
        changeUser(index, id, newName) {
            axios.put(`${API_URL}/api/v2/user/${id}`, { "name": newName })
                .then(res => {
        ...
        getCities() {
            axios.get("${API_URL}/api/v2/city/all/sorted")
                .then((res) => {
                    this.cities = res.data
                })
        },
        deleteCity(index, id) {
            axios.delete(`${API_URL}/api/v2/city/${id}`)
                .then(() => {
                    this.cities.splice(index, 1)
                })
        },
        changeCity(index, id, newName) {
            axios.put(`${API_URL}/api/v2/city/${id}`, { "cityName": newName })
                .then(res => {
        ...
        addCity() {
            axios.post('${API_URL}/api/v2/city/', { "cityName": this.cityName })
                .then(res => {
        ...
        addCityToFavorites() {
            axios.post('${API_URL}/api/v2/city/', { "cityName": this.cityName })
                .then(cityRes => {
                    axios.post(`${API_URL}/api/v2/user/add_city?user_id=${this.selectedUser}&city_id=${cityRes.data.id}`)
                        .then(() => {
                            this.getCities()
                            this.getUserCities()
                            this.goodAdd = true
        ...
        getUserCities() {
            axios.get(`${API_URL}/api/v2/user/${this.selectedUser}`)
                .then(res => {
